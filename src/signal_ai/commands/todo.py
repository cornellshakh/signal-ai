...
async def handle_todo(
    ...
    """
    if not args:
        await c.reply("Usage: `!todo [add|list|done] [args...]`")
        return
    ...
    if sub_command == "add":
        ...
        try:
            ...
            if 0 <= index < len(chat_context.todos):
                ...
                persistence_manager.save_context(c.message.source)
                await c.reply(f"Completed to-do: `{item}`")
            else:
                await c.reply("Invalid to-do index.")
        except ValueError:
            await c.reply("Invalid to-do index.")
    else:
        await c.reply(
