...
class AIClient:
    ...
    async def generate_response(
        ...
        try:
            ...
            log.info("ai.send.start", chat_id=chat_id)
            response = await chat_session.send_message_async(prompt)
            log.info("ai.send.end", chat_id=chat_id)
            await self._update_caches(chat_id, chat_session.history)
            if response.candidates:
                log.info("ai.response.success", chat_id=chat_id)
    ...
    async def _update_caches(self, chat_id: str, history: List[Any]):
        """
        ...
        try:
            ...
            await self._redis.set(chat_id, history_json)
    ...
    async def clear_chat_session(self, chat_id: str):
        """
